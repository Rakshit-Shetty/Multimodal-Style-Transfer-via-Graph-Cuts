...
def data_term(content_feature, cluster_centres):
	c = content_feature.permute(1, 2, 0)
	d = torch.matmul(c, cluster_centres)
	c_norm = torch.norm(c, dim=2, keepdim=True)
	s_norm = torch.norm(cluster_centres, dim=0, keepdim=True)
	norm = torch.matmul(c_norm, s_norm)
	d = 1 - d.div(norm)
	return d

def pairwise_term(cluster_centres, lam):
	_, k = cluster_centres.shape
	v = torch.ones((k,k)) - torch.eye(k)
	v = lam * v.to(cluster_centres.device)
	return v

def labelled_whiten_and_color(f_c, f_s, alpha, label):
	try:
		c, h, w = f_c.shape
...
class MST:
	...
	def style_feature_clustering(self, style_feature):
		...
		s = style_feature.reshape(C, -1).transpose(0, 1).to('cpu').numpy()
	...
	def graph_based_style_matching(self, content_feature, style_feature):
		cluster_centres, s_clusters = self.style_feature_clustering(style_feature)

		D = data_term(content_feature, cluster_centres).to('cpu').numpy().astype(np.double)
		V = pairwise_term(cluster_centres, lam=self.lam).to('cpu').numpy().astype(np.double)
